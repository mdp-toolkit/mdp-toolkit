...
def test_transformer_node():
    ...
    tr_node = TransformerNode(input_shape=inp_shape, transform_seq=['set_shape', 'gray', 'img_255_1', 'center'])
    out = tr_node(inp)
    assert(out.max() <= 1)
    assert(out.min() >= 0)
    ...
    # check transpose
    tr_node = TransformerNode(input_shape=inp_shape, transform_seq=['set_shape', 'transpose'])
    out = tr_node(inp)
...
def test_sklearn_transform_fns():
    ...
    tr_node = TransformerNode(input_shape=(3, 3, 3), transform_seq=['set_shape', 'gray', 'to_2d', 'binarize'])
...
def test_cv_pil_transform_fns():
    ...
    # check with a sequence of transformation
    tr_node = TransformerNode(input_shape=inp_shape, transform_seq=['set_shape', 'resize'],
