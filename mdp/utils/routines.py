...
def _legacysignature(signature):
    """Used to replace inspect.formatargspec by inspect.signature.
    
    For retrocompatibility reasons, we don't use a standard Signature.
    Instead, we use the string generated by this method.
    Basically, from a Signature we create a string and remove the default
    values.
    """
